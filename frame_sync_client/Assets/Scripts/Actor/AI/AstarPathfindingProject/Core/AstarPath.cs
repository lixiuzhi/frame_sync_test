...
public class AstarPath : MonoBehaviour
{
    ...
    private bool DrawUnwalkableNode(GraphNode node)
    {
        if (!node.Walkable)
        {
            Gizmos.DrawCube(node.position.vec3, Vector3.one * unwalkableNodeDebugSize);
        }
        return true;
    }
...
#line default
    ...
    public static bool StartPath(Path p, bool pushToFront = false)
    {
        ...
        {
            p.Error();
            Logger.err("No new paths are accepted");
            return false;
        }
        ...
        if (astar.graphs == null || astar.graphs.Length == 0)
        {
            ...
            p.Error();
            Logger.err("There are no graphs in the scene");
    ...
    public GraphNode GetNearest(Ray ray)
    {
        ...
        for (int i = 0; i < graphs.Length; i++)
        {
            ...
            graph.GetNodes(node => {
                Vector3 pos = node.position.vec3;
