...
public class GraphNodeRasterizer
{
	...
	public void Init(Int2 pos, int sizeX, int sizeY, int inCellSize)
	{ 
	...
	public void AddTriangle(ref Int2 v0, ref Int2 v1, ref Int2 v2, object data)
	{
	...
	private void AddLine(int x0, int x1, int y, object data)
	{
		...
		for (int i = x0; i <= x1; i++)
		{
			...
			{
				Logger.err( "index of rasterizer cells is out of range !");
			}
	...
	public List<object> GetLocated(Int3 pos)
	{
	...
	public void GetCellPosClamped(out int x, out int y, Int3 pos)
	{
	...
	public bool IntersectionSegment(int x, int y, Int3 start, Int3 end)
	{
		...
		for (int i = 0; i < this.numCellsY; i++)
		{
			for (int j = 0; j < this.numCellsX; j++)
			{
				...
				if (this.cells[num2] != null)
				{
					...
					if (obj != null)
					{
						float num3 = (float)(j * this.cellSize + this.origin.x + this.cellSize / 2) * 0.001f;
						float num4 = (float)(i * this.cellSize + this.origin.y + this.cellSize / 2) * 0.001f;
						Gizmos.color = this.debugColors[i % 2 * 2 + j % 2];
						Gizmos.DrawCube(new Vector3(num3, 0f, num4), vector); 
					}
				}
			}
		}
