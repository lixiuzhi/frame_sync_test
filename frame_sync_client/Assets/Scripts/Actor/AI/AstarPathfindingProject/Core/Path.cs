...
namespace Pathfinding
{
    public abstract class Path
    {
        ...
        public uint CalculateHScore(GraphNode node)
        {
            switch (this.heuristic)
            {
                ...
                case Heuristic.DiagonalManhattan:
                    {
                        Int3 vInt = this.GetHTarget() - node.position;
        ...
        public virtual void OnEnterPool()
        {
            if (this.vectorPath != null)
            {
                ListPool<Int3>.Release(this.vectorPath);
            }
            if (this.path != null)
            {
        ...
        public virtual void Reset()
        {
            ...
            {
                Logger.err("No AstarPath object found in the scene. Make sure there is one or do not create paths in Awake");
            }
            ...
            this.vectorPath = ListPool<Int3>.Claim();
            this.currentR = null;
            this.duration = 0f;
        ...
        protected virtual void Trace(PathNode from)
        {
            ...
            while (pathNode != null)
            {
                ...
                if (num > 1024)
                { 
                    Logger.wrn("Inifinity loop? >1024 node path. Remove this message if you really have that long paths (Path.cs, Trace function)");
                    break;
                }
            }
            ...
            {
                GraphNode graphNode = this.path[j];
                this.path[j]= this.path[num - j - 1];
                this.path[num - j - 1]= graphNode;
            }
            for (int k = 0; k < num; k++)
            {
