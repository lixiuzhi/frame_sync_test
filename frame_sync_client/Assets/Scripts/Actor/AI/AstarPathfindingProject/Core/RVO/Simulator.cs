...
namespace Pathfinding.RVO
{
	public class Simulator
	{
		...
		internal class WorkerContext
		{
			...
			public Int2[] bestPos = new Int2[3];

			public long[] bestSizes = new long[3];

			public IntFactor[] bestScores = new IntFactor[4];

			public Int2[] samplePos = new Int2[50];
		...
		public IntFactor qualityCutoff = new IntFactor
		{
			numerator = 1L,
			denominator = 20L
		};
		...
		private IntFactor wallThickness = IntFactor.one;
		...
		public IntFactor WallThickness
		{
			get
			{
		...
		public Agent AddAgent(Int3 position)
		{
		...
		public ObstacleVertex AddObstacle(Int3[] vertices, int height)
		{
			return this.AddObstacle(vertices, height, Matrix4x4.identity, RVOLayer.DefaultObstacle);
		}

		public ObstacleVertex AddObstacle(Int3[] vertices, int height, Matrix4x4 matrix, RVOLayer layer = RVOLayer.DefaultObstacle)
		{
			if (vertices == null)
			{
			...
			for (int j = 0; j < vertices.Length; j++)
			{
				...
				obstacleVertex3.position = ((!flag) ? ((Int3)matrix.MultiplyPoint3x4((Vector3)vertices[j])) : vertices[j]);
		...
		public ObstacleVertex AddObstacle(Int3 a, Int3 b, Int1 height)
		{
			...
			Int2 vInt = new Int2(b.x - a.x, b.z - a.z);
			vInt.Normalize();
