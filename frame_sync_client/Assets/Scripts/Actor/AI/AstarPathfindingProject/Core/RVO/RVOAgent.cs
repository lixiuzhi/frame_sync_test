...
namespace Pathfinding.RVO.Sampled
{
    public class Agent : IAgent
    {
        public struct VO
        {
            ...
            private IntFactor weightFactor;

            public VO(Int2 offset, Int2 p0, Int2 dir, IntFactor weightFactor)
            {
                this.colliding = true;
                this.line1 = p0;
                this.dir1 = -dir;
                ...
                this.leftSide = false;
                this.radius = 0;
                this.weightFactor.numerator = weightFactor.numerator;
                this.weightFactor.denominator = weightFactor.denominator << 1;
            }

            public VO(Int2 offset, Int2 p1, Int2 p2, Int2 tang1, Int2 tang2, IntFactor weightFactor)
            {
                this.weightFactor.numerator = weightFactor.numerator;
                this.weightFactor.denominator = weightFactor.denominator << 1;
                ...
                this.leftSide = false;
                this.radius = 0;
                weightFactor.numerator = 5L;
                weightFactor.denominator = 1L;
            }

            public VO(Int2 center, Int2 offset, int radius, Int2 sideChooser, IntFactor inverseDt, IntFactor weightFactor)
            {
                this.weightFactor.numerator = weightFactor.numerator;
                this.weightFactor.denominator = weightFactor.denominator << 1;
                this.origin = offset;
                weightFactor.numerator = 1L;
                weightFactor.denominator = 2L;
                ...
                if (sqrMagnitudeLong < num2)
                {
                    ...
                    Int2 vInt = new Int2(this.line1.y, -this.line1.x);
                ...
                else
                {
                    ...
                    Int2 b = center + offset;
                    this.sqrCutoffDistance = (long)(num - radius);
                    this.center = center;
                    this.cutoffLine = IntMath.Divide(a, this.sqrCutoffDistance, 1000L);
                    Int2 vInt2 = new Int2(-this.cutoffLine.y, this.cutoffLine.x);
                    this.cutoffDir = vInt2.normalized;
                    this.cutoffLine += offset;
                    this.sqrCutoffDistance *= this.sqrCutoffDistance;
                    IntFactor a2 = IntMath.atan2(-center.y, -center.x);
                    IntFactor b2 = IntMath.acos((long)radius, (long)num);
                    this.radius = radius;
                    this.leftSide = Polygon.Left(Int2.zero, center, sideChooser);
                    IntFactor f;
                    IntFactor f2;
                    IntMath.sincos(out f, out f2, a2 + b2);
                    this.line1 = new Int2(radius * f2, radius * f);
                    Int2 vInt3 = new Int2(this.line1.y, -this.line1.x);
                    this.dir1 = vInt3.normalized;
                    IntMath.sincos(out f, out f2, a2 - b2);
                    this.line2 = new Int2(radius * f2, radius * f);
                    Int2 vInt4 = new Int2(this.line2.y, -this.line2.x);
            ...
            public Int2 Sample(Int2 p, out IntFactor weight)
            {
                if (this.colliding)
                {
                    ...
                    if (num >= 0L)
                    {
                        weight.numerator = this.weightFactor.numerator * num;
                        weight.denominator = this.weightFactor.denominator * 1000L;
                        return IntMath.Divide(new Int2(-this.dir1.y, this.dir1.x), weight.numerator * (long)Agent.GlobalIncompressibility, weight.denominator);
                    }
                    weight = IntFactor.zero;
                    return Int2.zero;
                }
                else
                {
                    ...
                    if (this.leftSide)
                    {
                        ...
                        {
                            weight.numerator = this.weightFactor.numerator * num2;
                            weight.denominator = this.weightFactor.denominator * 1000L;
                            return new Int2(-this.cutoffDir.y, this.cutoffDir.x) * weight;
                        }
                        weight.numerator = this.weightFactor.numerator * num3;
                        weight.denominator = this.weightFactor.denominator * 1000L;
                    ...
                    else
                    {
                        ...
                        weight.numerator = this.weightFactor.numerator * num4;
                        weight.denominator = this.weightFactor.denominator * 1000L;
            ...
            public IntFactor ScalarSample(Int2 p)
            {
                if (this.colliding)
                {
                    long num = Agent.VO.Det(this.line1, this.dir1, p);
                    if (num >= 0L)
                    {
                        return new IntFactor(num * (long)Agent.GlobalIncompressibility * this.weightFactor.numerator, this.weightFactor.denominator * 1000L);
                    }
                    return IntFactor.zero;
                }
                else
                {
                    long num2 = Agent.VO.Det(this.cutoffLine, this.cutoffDir, p);
                    ...
                    if (this.leftSide)
                    {
                        if (num2 < (long)(this.radius * 1000))
                        {
                            return new IntFactor(num2 * this.weightFactor.numerator, this.weightFactor.denominator * 1000L);
                        }
                        return new IntFactor(num3 * this.weightFactor.numerator, this.weightFactor.denominator * 1000L);
                    }
                    else
                    {
                        ...
                        return new IntFactor(num4 * this.weightFactor.numerator, this.weightFactor.denominator * 1000L);
                    }
                }
            }
        }
        ...
        public Int1 agentTimeHorizon;

        public Int1 obstacleTimeHorizon;

        public Int1 radius;

        public Int1 height;

        public Int1 maxSpeed;

        public Int1 neighbourDist;
        ...
        private static IntFactor adpSamp_Steps2_InnerScale;

        private static IntFactor[] adpSamp_Steps3_Sin;

        private static IntFactor[] adpSamp_Steps3_Cos;
        ...
        public static Int1 WallWeight;

        private static List<IntLine> orcaLines;

        private static List<IntLine> projLines;
        ...
        static Agent()
        {
            Agent.orcaLines = new List<IntLine>();
            Agent.projLines = new List<IntLine>();
            ...
            Agent.adpSamp_Steps2_InnerScale = new IntFactor(3L, 5L);
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                vFactor2.numerator += vFactor2.denominator;
                Agent.adpSamp_Steps1_Pos_Nom1[i] = vFactor.numerator * vFactor2.denominator;
                Agent.adpSamp_Steps1_Pos_Nom2[i] = vFactor.denominator * vFactor2.numerator;
                Agent.adpSamp_Steps1_Pos_Den[i] = vFactor.denominator * vFactor2.denominator;
                Agent.adpSamp_Steps1_Size_Nom[i] = 8 - Mathf.Abs(i - 4);
            }
            IntFactor inverse = Agent.adpSamp_Steps2_InnerScale.Inverse;
            ...
            for (int j = 0; j < 6; j++)
            {
                ...
                vFactor += inverse;
                Agent.adpSamp_Steps2_Den[j] = vFactor2.denominator * vFactor.denominator * Agent.adpSamp_Steps2_InnerScale.denominator;
                Agent.adpSamp_Steps2_Nom_rw[j] = vFactor2.numerator * vFactor.denominator;
                Agent.adpSamp_Steps2_Nom_fw[j] = vFactor.numerator * vFactor2.denominator;
            }
            Agent.adpSamp_Steps3_Sin = new IntFactor[6];
            Agent.adpSamp_Steps3_Cos = new IntFactor[6];
            for (int k = 0; k < 6; k++)
            {
                IntFactor vFactor;
                IntFactor vFactor2;
                IntMath.sincos(out vFactor, out vFactor2, (long)((2 * k + 1) * 31416), 60000L);
                vFactor.denominator *= 5L;
                vFactor2.denominator *= 5L;
        ...
        public void SetYPosition(Int1 yCoordinate)
        {
        ...
        private static void DrawCircle(Vector2 _p, float radius, float a0, float a1, Color col)
        {
            ...
            while ((float)num <= 40f)
            {
                Vector3 vector3 = new Vector3(Mathf.Cos(Mathf.Lerp(a0, a1, (float)num / 40f)) * radius, 0f, Mathf.Sin(Mathf.Lerp(a0, a1, (float)num / 40f)) * radius);
                vector2 = vector3;
                num++;
            }
        }
        ...
        internal void CalculateVelocity(Simulator.WorkerContext context)
        {
            ...
            Int2 b = new Int2(this.position.x, this.position.z);
            Agent.VO[] vos = context.vos;
            int num = 0;
            Int2 vInt = new Int2(this.velocity.x, this.velocity.z);
            IntFactor vFactor = new IntFactor(1000L, (long)this.agentTimeHorizon.i);
            IntFactor one = IntFactor.one;
            if (this.simulator.algorithm != Simulator.SamplingAlgorithm.GradientDecent)
            {
                one.numerator = (long)Agent.WallWeight.i;
                one.denominator = 1000L;
            }
            IntFactor vFactor2 = one;
            vFactor2.numerator <<= 1;
            IntFactor vFactor3 = one;
            vFactor3.denominator *= 20L;
            this.CalcVelocity_Neighbours(ref b, vos, ref num, ref vInt, ref vFactor);
            Int2 vInt2 = Int2.zero;
            if (num > 0)
            {
                if (this.simulator.algorithm == Simulator.SamplingAlgorithm.GradientDecent)
                {
                ...
                sampleSize[num2] = (long)(num3 * 3 / 10);
                num2++;
                Int2 fw = new Int2(vInt.x >> 1, vInt.y >> 1);
            ...
            vInt2 = Int2.ClampMagnitude(vInt2, this.maxSpeed.i);
            this.newVelocity = new Int3(vInt2.x, 0, vInt2.y);
        }

        private void CalcVelocity_Neighbours(ref Int2 position2D, Agent.VO[] vos, ref int voCount, ref Int2 optimalVelocity, ref IntFactor inverseAgentTimeHorizon)
        {
            for (int i = 0; i < this.neighbours.Count; i++)
            {
                ...
                if (agent != this)
                {
                    ...
                    if (num - num2 >= 0)
                    {
                        ...
                        Int1 vInt = this.radius + agent.radius;
                        Int2 vInt2 = agent.position.xz - position2D;
                        ...
                        vos[voCount] = new Agent.VO(vInt2, vInt3, vInt.i, sideChooser, inverseAgentTimeHorizon, IntFactor.one);
                        voCount++;
                        if (this.DebugDraw)
                        {
                            Int2 b = IntMath.Divide(vInt2, inverseAgentTimeHorizon.numerator, inverseAgentTimeHorizon.denominator);
                            Int1 ob = (int)IntMath.Divide((long)vInt.i * inverseAgentTimeHorizon.numerator, inverseAgentTimeHorizon.denominator);
        ...
        private void CalcVelocity_Obstacles(ref Int2 position2D, Agent.VO[] vos, ref int voCount, ref IntFactor wallThickness, ref IntFactor wallWeight, ref IntFactor wallWeight2, ref IntFactor wallWeight3)
        {
            for (int i = 0; i < this.simulator.obstacles.Count; i++)
            {
                ...
                do
                {
                    ...
                    else
                    {
                        ...
                        long num2 = num;
                        long b = Int2.DotLong(obstacleVertex2.dir, position2D - obstacleVertex2.position.xz);
                        ...
                        if (Math.Abs(num2) < (long)(this.neighbourDist.i * 1000))
                        {
                            ...
                            {
                                Int2 p = obstacleVertex2.position.xz - position2D;
                                Int2 p2 = obstacleVertex2.next.position.xz - position2D;
                                Int2 normalized = p.normalized;
                                Int2 normalized2 = p2.normalized;
        ...
        private static void CalcSamplePos_Step1(Int2[] samplePos, long[] sampleSize, ref int samplePosCount, int sampleScale, Int2 rw, Int2 fw)
        {
            ...
            vFactor.denominator = 16L;
            for (int i = 0; i < 8; i++)
            {
                vFactor.numerator = (long)(Agent.adpSamp_Steps1_Size_Nom[i] * sampleScale);
        ...
        private static void CalcSamplePos_Step2(Int2[] samplePos, long[] sampleSize, ref int samplePosCount, int sampleScale, ref Int2 fw, ref Int2 rw)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                samplePos[samplePosCount].x = (int)IntMath.Divide(((long)rw.x * Agent.adpSamp_Steps2_Nom_rw[i] + (long)fw.x * Agent.adpSamp_Steps2_Nom_fw[i]) * Agent.adpSamp_Steps2_InnerScale.numerator, Agent.adpSamp_Steps2_Den[i]);
                samplePos[samplePosCount].y = (int)IntMath.Divide(((long)rw.y * Agent.adpSamp_Steps2_Nom_rw[i] + (long)fw.y * Agent.adpSamp_Steps2_Nom_fw[i]) * Agent.adpSamp_Steps2_InnerScale.numerator, Agent.adpSamp_Steps2_Den[i]);
                sampleSize[samplePosCount] = (long)num;
                samplePosCount++;
            }
        }

        private void CalcSamplePosScore(Simulator.WorkerContext context, ref Int2 position2D, Agent.VO[] vos, int voCount, ref Int2 optimalVelocity, ref Int2 result, Int2[] samplePos, long[] sampleSize, ref int samplePosCount, ref Int2 desired2D)
        {
            Int2[] bestPos = context.bestPos;
            long[] bestSizes = context.bestSizes;
            IntFactor[] bestScores = context.bestScores;
            for (int i = 0; i < 3; i++)
            {
                bestScores[i] = new IntFactor(2147483647L, 1L);
            }
            bestScores[3] = new IntFactor(-2147483648L, 1L);
            Int2 vInt = optimalVelocity;
            IntFactor b = new IntFactor(2147483647L, 1L);
            for (int j = 0; j < 3; j++)
            {
                for (int k = 0; k < samplePosCount; k++)
                {
                    IntFactor vFactor = IntFactor.zero;
                    for (int l = 0; l < voCount; l++)
                    {
                        IntFactor vFactor2 = vos[l].ScalarSample(samplePos[k]);
                        vFactor = ((!(vFactor > vFactor2)) ? vFactor2 : vFactor);
                    }
                    int magnitude = (samplePos[k] - desired2D).magnitude;
                    IntFactor vFactor3 = vFactor + new IntFactor((long)magnitude, 5L);
                    vFactor += new IntFactor((long)magnitude, 1000L);
                    if (this.DebugDraw)
                    {
                ...
                {
                    Int2 vInt2 = bestPos[num];
                    long num2 = bestSizes[num];
                    bestScores[num] = new IntFactor(2147483647L, 1L);
        ...
        private Int2 Trace(Agent.VO[] vos, int voCount, Int2 p, IntFactor cutoff, out float score)
        {
            score = 0f;
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                {
                    IntFactor vFactor2;
                    Int2 b = vos[j].Sample(p, out vFactor2);
                    vInt += b;
                    if (vFactor2 > vFactor)
                    {
                        vFactor = vFactor2;
                ...
                IntFactor vFactor3 = new IntFactor((long)a.magnitude, 500L);
                ...
                {
                    IntFactor f = vFactor;
                    f.denominator *= (long)IntMath.Sqrt((long)sqrMagnitude);
                    vInt *= f;
                }
                vInt = (Int2)((Vector2)vInt * num2);
                Int2 ob = p + vInt;
                if (this.DebugDraw)
                {
                    UnityEngine.Debug.DrawLine(Agent.To3D((Vector2)ob) + (Vector3)this.position, Agent.To3D((Vector2)p) + (Vector3)this.position, Agent.Rainbow(0.1f / score) * new Color(1f, 1f, 1f, 0.2f));
                }
            }
            score = num;
            return result;
        ...
        public static bool IntersectionFactor(Int2 start1, Int2 dir1, Int2 start2, Int2 dir2, out IntFactor factor)
        {
            ...
            if (num3 == 0L)
            {
                factor = IntFactor.zero;
                return false;
            }
            long nom = num * (long)(start1.y - start2.y) - num2 * (long)(start1.x - start2.x);
            factor.numerator = nom;
            factor.denominator = num3;
            return true;
        }
        ...
        public Int3 FixVelocity(Int2 result)
        {
            ...
            return new Int3(vInt.x, 0, vInt.y);
        }
        ...
        public void computeNewVelocity()
        {
            ...
            int numObstLines = 0;
            IntFactor f = new IntFactor(1000L, (long)this.AgentTimeHorizon);
            Int2 zero = Int2.zero;
            IntLine vLine = default(IntLine);
            Int2 a = default(Int2);
            for (int i = 0; i < this.neighbours.Count; i++)
            {
                Agent agent = this.neighbours[i];
                Int2 xz = (agent.position - this.position).xz;
                Int2 xz2 = (this.velocity - agent.velocity).xz;
                long num = Int2.DotLong(ref xz, ref xz);
                ...
                if (num > num3)
                {
                    ...
                    long num4 = Int2.DotLong(ref vInt, ref xz);
                    if (num4 < 0L && num4 * num4 > num3 * sqrMagnitudeLong)
                    {
                        ...
                        long num5 = IntMath.Divide(f.numerator * num2, f.denominator) - (long)IntMath.Sqrt(sqrMagnitudeLong);
                    ...
                    else
                    {
                        ...
                        if (Int2.DetLong(ref xz, ref vInt) > 0L)
                        {
                        ...
                        long num7 = Int2.DotLong(ref xz2, ref vLine.direction);
                ...
                {
                    this.hasCollided = true;
                    IntFactor f2 = new IntFactor(1000L, 30L);
                    Int2 vInt3 = xz2 - xz * f2;
                    Int2 normalized = vInt3.normalized;
                    vLine.direction = new Int2(normalized.y, -normalized.x);
                    long num8 = IntMath.Divide(num2 * f2.numerator, f2.denominator) - (long)vInt3.magnitude;
        ...
        private static bool linearProgram1(List<IntLine> lines, int lineNo, int radius, Int2 optVelocity, bool directionOpt, ref Int2 result)
        {
            IntLine vLine = lines[lineNo];
            long num = IntMath.Divide(Int2.DotLong(ref vLine.point, ref vLine.direction), 1000L);
            ...
            IntFactor vFactor = new IntFactor(-num - num3, 1L);
            IntFactor vFactor2 = new IntFactor(-num + num3, 1L);
            IntFactor vFactor3 = default(IntFactor);
            for (int i = 0; i < lineNo; i++)
            {
                IntLine vLine2 = lines[i];
                vFactor3.denominator = Int2.DetLong(ref vLine.direction, ref vLine2.direction);
                vFactor3.numerator = Int2.DetLong(vLine2.direction, vLine.point - vLine2.point);
                if (vFactor3.denominator == 0L)
                {
                    if (vFactor3.numerator < 0L)
                    {
                        return false;
                    }
                }
            ...
            if (directionOpt)
            {
                if (Int2.DotLong(ref optVelocity, ref vLine.direction) > 0L)
                {
            ...
            {
                IntFactor b = new IntFactor(Int2.DotLong(vLine.direction, optVelocity - vLine.point), 1000L);
                if (vFactor > b)
                {
                    result = vLine.point + IntMath.Divide(vLine.direction * vFactor, 1000L);
                }
                ...
                {
                    result = vLine.point;
                    result.x += (int)IntMath.Divide((long)vLine.direction.x * b.numerator, b.denominator * 1000L);
                    result.y += (int)IntMath.Divide((long)vLine.direction.y * b.numerator, b.denominator * 1000L);
                }
            }
            return true;
        }

        private static int linearProgram2(List<IntLine> lines, int radius, Int2 optVelocity, bool directionOpt, ref Int2 result)
        {
            if (directionOpt)
            {
            ...
            {
                IntLine vLine = lines[i];
                if (Int2.DetLong(vLine.direction, vLine.point - result) > 0L)
                {
                    Int2 vInt = result;
        ...
        private static void linearProgram3(List<IntLine> lines, int numObstLines, int beginLine, int radius, ref Int2 result)
        {
            long num = 0L;
            for (int i = beginLine; i < lines.Count; i++)
            {
                IntLine vLine = lines[i];
                if (Int2.DetLong(vLine.direction, vLine.point - result) > num)
                {
                    ...
                    while (k < i)
                    {
                        IntLine vLine2 = lines[k];
                        long num2 = Int2.DetLong(vLine.direction, vLine2.direction);
                        IntLine vLine3;
                        if (num2 != 0L)
                        {
                            Int2 b = IntMath.Divide(vLine.direction, Int2.DetLong(vLine2.direction, vLine.point - vLine2.point), num2);
                            vLine3.point = vLine.point + b;
                            goto IL_13C;
                        }
                        if (Int2.DetLong(vLine.direction, vLine2.direction) <= 0L)
                        {
                    ...
                    Int2 vInt = result;
                    if (Agent.linearProgram2(Agent.projLines, radius, new Int2(-vLine.direction.y, vLine.direction.x), true, ref result) < Agent.projLines.Count)
                    {
                        result = vInt;
                    }
                    Agent.projLines.Clear();
                    num = Int2.DetLong(vLine.direction, vLine.point - result);
                }
            }
        }
    }
