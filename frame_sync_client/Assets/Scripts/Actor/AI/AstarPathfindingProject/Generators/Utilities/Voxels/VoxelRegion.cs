...
namespace Pathfinding.Voxels {
	public partial class Voxelize {
		...
		public void BuildRegions () {
			...
			List<IntVector3> basins = Pathfinding.Util.ListPool<IntVector3>.Claim(100);
			...
			for (int z = 0, pz = 0; z < wd; z += w, pz++) {
				for (int x = 0; x < voxelArea.width; x++) {
					...
					for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; i++) {
						...
						if (!anyBelow) {
							basins.Add(new IntVector3(x, i, z));
			...
			List<IntVector3> st1 = Pathfinding.Util.ListPool<IntVector3>.Claim(300);
			List<IntVector3> st2 = Pathfinding.Util.ListPool<IntVector3>.Claim(300);
					...
					for (int dir = 0; dir < 4; dir++) {
						...
						if (srcReg[ni] == 0) {
							...
							if (!same) {
								...
								st1.Add(new IntVector3(nx, ni, nz));
							}
						}
					}
			...
			Pathfinding.Util.ListPool<IntVector3>.Release(st1);
			Pathfinding.Util.ListPool<IntVector3>.Release(st2);
			Pathfinding.Util.ListPool<IntVector3>.Release(basins);
