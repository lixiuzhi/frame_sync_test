...
namespace Pathfinding {
	...
	public abstract class NavGraph {
        ...
        {
            return GetNearest(position.vec3, NNConstraint.None);
        }
        ...
        {
            return GetNearest(position.vec3, constraint, null);
        }
        public virtual NNInfo GetNearest(Int3 position, NNConstraint constraint, GraphNode hint)
        {
            return GetNearest(position.vec3,  constraint,  hint);
        }
        /** Returns the nearest node to a position using the specified NNConstraint.
		 * \param position The position to try to find a close node to
        ...
        public virtual NNInfo GetNearest (Vector3 position, NNConstraint constraint, GraphNode hint) {
			...
			if (minConstNode != null) {
				nnInfo.constClampedPosition = minConstNode.position.vec3;
			} else if (minNode != null) {
				nnInfo.constrainedNode = minNode;
				nnInfo.constClampedPosition = minNode.position.vec3;
			}
        ...
        public virtual NNInfo GetNearestForce(Int3 position, NNConstraint constraint)
        {
            return GetNearest(position.vec3, constraint);
        }
        /**
