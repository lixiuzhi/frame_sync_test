...
namespace Pathfinding {
	...
	public class RecastGraph : NavGraph, INavmesh, IRaycastableGraph, IUpdatableGraph, INavmeshHolder {
		...
		protected IEnumerable<Progress> ScanAllTiles () {
			...
			for (int i = 0; i < tiles.Length; i++) tileQueue.Enqueue(new IntVector2(tiles[i].x, tiles[i].z));
		...
		void ConnectTiles (NavmeshTile tile1, NavmeshTile tile2) {
			...
			v1[coord] = midpoint*IntVector3.PrecisionFactor;
			v2[coord] = midpoint*IntVector3.PrecisionFactor;
		...
		public void ReplaceTile (int x, int z, int w, int d, Int3[] verts, int[] tris, bool worldSpace) {
			...
			//Profiler.BeginSample("Replace Tile Init");
		 ...
		 * \astarpro */
		public bool Linecast (Int3 tmp_origin, Int3 tmp_end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace) {
