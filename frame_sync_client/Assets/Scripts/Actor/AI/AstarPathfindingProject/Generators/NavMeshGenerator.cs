...
namespace Pathfinding {
	...
	public class NavMeshGraph : NavGraph, INavmesh, IUpdatableGraph, INavmeshHolder
			...
			GraphNodeDelegateCancelable del = delegate(GraphNode _node) {
				...
				if (accurateNearestNode) {
					...
					if (!node.ContainsPoint((Int3)position)) {
						float dist = (float)(node.position-pos).sqrMagnitude;
						if (minNode == null || dist < minDist) {
							minDist = dist;
							minNode = node;
						}
		...
		public bool Linecast (Int3 origin, Int3 end) {
			return Linecast(origin, end, GetNearest(origin.vec3, NNConstraint.None).node);
		}
		...
		public static bool Linecast (INavmesh graph, Int3 tmp_origin, Int3 tmp_end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace) {
            ...
            vInt2 = (Int3)triangleMeshNode.ClosestPointOnNode((Vector3)vInt2);
            hit.origin = vInt2;
            ...
            for (int j = 0; j < vertexCount; j++)
            {
                ...
                if (Polygon.LeftNotColinear(vertex, vertex2, hit.origin) || !Polygon.LeftNotColinear(vertex, vertex2, tmp_end))
                {
                    ...
                    if (Polygon.IntersectionFactor(vertex, vertex2, hit.origin, tmp_end, out vFactor, out vFactor2))
                    {
                        ...
                        {
                            if (!vFactor.IsNegative && vFactor.numerator / vFactor.denominator <= 1L)
                            {
                                Int3 vInt5 = (vertex2 - vertex) * (float)vFactor.numerator;
                                vInt5 = IntMath.Divide(vInt5, vFactor.denominator);
                                vInt5 += vertex;
                                hit.point = vInt5;
                                hit.node = triangleMeshNode;
